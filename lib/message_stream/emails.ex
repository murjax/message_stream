defmodule MessageStream.Emails do
  @moduledoc """
  The Emails context.
  """
  ...
  def list_emails(offset: offset, limit: limit, search: nil) do
    from(e in Email, limit: ^limit, offset: ^offset)
    |> Repo.all()
  end

  def list_emails(offset: offset, limit: limit, search: search) do
    from(
      e in Email,
      limit: ^limit,
      offset: ^offset,
      where: like(e.from_email, ^"%#{search}%")
    )
    |> Repo.all()
  end
